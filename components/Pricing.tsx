...
const pricingData: PricingPlan[] = [
  ...
  },
  {
    ...
    description: "Diseñamos un sistema que automatiza un proceso repetitivo con IA. ROI promedio del 400% en 6 meses.",
    features: [
...
const PricingCard: React.FC<PricingPlan & { billingCycle: BillingCycle }> = ({ name, type, price, description, features, featured, cta, href, billingCycle }) => {
    
    const displayPrice = () => {
        if (type === 'subscription') {
            const priceData = price as { monthly: number; annual: number };
            const amount = billingCycle === 'monthly' ? priceData.monthly : Math.round(priceData.annual / 12);
            return (
    ...
    return (
        <div className={`glow-card h-full ${featured ? 'featured-glow' : ''}`}>
            <div className={`relative z-10 bg-[#0f1334] p-8 rounded-2xl h-full flex flex-col ${featured ? 'border-2 border-cyan-400' : 'border border-slate-700/50'}`}>
            ...
            <a href={href} className={`w-full block text-center px-6 py-3 mt-auto rounded-full font-bold transform transition-all duration-300 ${featured ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white hover:scale-105' : 'border-2 border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-[#0a0e27]'}`}>
                {cta}
            </a>
            </div>
        </div>
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="flex justify-center mb-16">
            <div className="bg-slate-800/50 p-1.5 rounded-full flex items-center space-x-2 border border-slate-700">
                ...
                >
                    Pago Anual
                    <span className="absolute -top-2.5 right-0 bg-green-400 text-[#0a0e27] text-xs font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">Ahorra 10%</span>
                </button>
            </div>
        </div>

        <div ref={gridRef} className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-8 items-stretch">
          {pricingData.map((plan, index) => (
            ...
            >
              {plan.featured && (
                <span className="absolute top-0 right-6 -translate-y-1/2 bg-cyan-400 text-[#0a0e27] px-4 py-1 rounded-full text-sm font-bold z-20">MÁS POPULAR</span>
              )}
